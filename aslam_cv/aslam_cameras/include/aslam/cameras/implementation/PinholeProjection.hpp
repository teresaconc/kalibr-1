...
bool PinholeProjection<DISTORTION_T>::initializeIntrinsics(const std::vector<GridCalibrationTargetObservation> &observations) {
  ...
  //Later Updated. Initialize for no errors in the future
  _fu = 300;
  _fv = 300;
  //process all images
  ...
  for (size_t i=0; i<nImages; ++i) {
    ...
    int count = 0;
    //If the grid is assymetric we never see all grid Points.
    //Initialization can't be the same as for the symmetric case.
    ...
    // Assymetric case: If a marker is detected there is at least one horizontal line to compute vanishing lines
    ...
    for (size_t j=0; j<target.rows(); ++j)
    {
      for (size_t k=j+1; k<target.cols(); ++k)
      {
        ...
        std::vector < cv::Point2d > ipts;
        // Account for cases where the initialization may fail due to wherever reason
